...
/* **************************************************************
 * Created by oscar oscar@decsai.ugr.es: lun 29 ago, 2005  11:57
 * Last modified: mi� 10 may, 2006  12:56
 * ************************************************************** */
...
STP::STP() {
   ...
   last_i = last_j = -1;
   FlagContextInfo = false;
   constraints = new vector<vector<int> >();
}
...
STP::~STP() {
  ...
  if(constraints)
      delete constraints;
  constraints = 0;
  for_each(constHistory.begin(),constHistory.end(),DeleteVector<vector<int> >());
  constHistory.clear();
}
...
int STP::InsertTPoint(const Task * t) {
    //cerr << "stp->InsertTPoint();" << endl;
    //cerr << net.size() << endl;
  vector <constr> p;
  constr tc=POS_INF;
...
bool STP::AddTConstraint(int i, int j, constr lower, constr upper, bool record) {
  //cerr << i << " " << j << " " << lower << " " << upper << endl;
	...
	if(FlagContextInfo){
	    int s = getmax(i,j);
	    if(s >= (int) constraints->size())
		constraints->resize(s+1);
...
bool STP::PathConsistency2() {
	...
	while (i < s) {
	   ...
	   // getchar();
		...
		if (uaux != u1 || laux != l1) {
		    if(FlagContextInfo){
			// comprobar si hay una restricci�n directa
			vector<int>::const_iterator ri, re = (*constraints)[i].end();
			bool encontrado = false;
			for(ri = (*constraints)[i].begin(); ri != re && !encontrado; ri++){
			    if((*ri) == j)
				encontrado = true;
			}

			re = (*constraints)[j].end();
			for(ri = (*constraints)[j].begin(); ri != re && !encontrado; ri++){
			    if((*ri) == i)
				encontrado = true;
			}
			...
			// comprobar si hay un v�nculo causal
			else {
			    a = table[i];
			    b = table[j];
			    if(causalTable.hasCausalLinks(a,b)){
				queue.push_back(make_pair(i,j));
				CL++;
			    }
			    else
...
bool STP::Schedule(vector <pair<int,int> > &solution) const {
  ...
  for (i=1; i<Size() && result; i++) {
	...
	upper = getmin(upper,Plus(solution[j].first,Query(j,i)));
...
void STP::SetLevel(void){
    ...
    history.push(clon);
    // guardar el tama�o del vector de re

    if(FlagContextInfo){
	vector<vector<int> > * v;
	v = new vector<vector<int> >(*constraints);
	constHistory.push_back(v);
    }
};
...
void STP::UnSetLevel(void){
	...
	if(FlagContextInfo){
	    vector<vector<int> > * v = constHistory.back();
	    constHistory.pop_back();
	    delete constraints;
	    constraints = v;
	}
    }
