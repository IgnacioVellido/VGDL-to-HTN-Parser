...
class ProblemGeneratorHPDL:
    ...
    def assign_init(self):
        ...
        for obj in self.levelObjects:
            ...
            self.init.append("(= (last_coordinate_x " + obj.name + ") " + str(obj.col) + ")")
            self.init.append("(= (last_coordinate_y " + obj.name + ") " + str(obj.row) + ")")
            ...
            for obj2 in self.levelObjects:
                if obj is not obj2:
                    self.init.append("(evaluate-interaction " + obj.name + " " + obj2.name + ")")
        ...
        for obj in all_counters:
            if obj:     # Hay un objeto None que no debería estar
                self.init.append("(= (counter_" + obj + ") " + str(all_counters[obj]) + ")")
    ...
    def parse_level(self):
        ...
        for tt in self.transformTo:
            for (lt, st, c) in zip(self.long_types, self.short_types, self.name_counters):
                if tt == lt:
