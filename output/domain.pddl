...
(define (domain VGDLGame)  
	...
	(:types
		...
		boulder - Passive
		wall - Immovable
		Immovable Passive Missile OrientedAvatar - Object
	)
	...
	(:task check-interactions
		...
		(:method green_avatar_killsprite
				:precondition (and 
									(evaluate-interaction ?x - green ?y - OrientedAvatar)
									(not (= (coordinate_x ?x) -1))
									(not (= (coordinate_x ?y) -1))
								)
				:tasks ( 
							(GREEN_AVATAR_KILLSPRITE ?x ?y)
							(:inline () (not (evaluate-interaction ?x - green ?y - OrientedAvatar)))
							(:inline () (regenerate-interaction ?x - green ?y - OrientedAvatar))
							(check-interactions) 
						)
		)
		...
		(:method blue_avatar_killsprite
				:precondition (and 
									(evaluate-interaction ?x - blue ?y - OrientedAvatar)
									(not (= (coordinate_x ?x) -1))
									(not (= (coordinate_x ?y) -1))
								)
				:tasks ( 
							(BLUE_AVATAR_KILLSPRITE ?x ?y)
							(:inline () (not (evaluate-interaction ?x - blue ?y - OrientedAvatar)))
							(:inline () (regenerate-interaction ?x - blue ?y - OrientedAvatar))
							(check-interactions) 
						)
		)
		...
		(:method red_avatar_killsprite
				:precondition (and 
									(evaluate-interaction ?x - red ?y - OrientedAvatar)
									(not (= (coordinate_x ?x) -1))
									(not (= (coordinate_x ?y) -1))
								)
				:tasks ( 
							(RED_AVATAR_KILLSPRITE ?x ?y)
							(:inline () (not (evaluate-interaction ?x - red ?y - OrientedAvatar)))
							(:inline () (regenerate-interaction ?x - red ?y - OrientedAvatar))
							(check-interactions) 
						)
		)
	...
	(:action AVATAR_WALL_STEPBACK
		...
		:effect (and 
					(assign (coordinate_y ?x) (last_coordinate_x ?x))
